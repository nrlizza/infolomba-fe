  ...
  <div class="p-6 md:p-8 max-w-7xl mx-auto">
    ...
    <div v-else class="overflow-x-auto rounded-lg border border-gray-200">
      <fwb-table class="min-w-full border border-blue-600">
        <fwb-table-head>
          <fwb-table-head-cell class="px-4 py-3 bg-[#4954DE] text-white border border-blue-300">Nama Lengkap</fwb-table-head-cell>
          <fwb-table-head-cell class="px-4 py-3 bg-[#4954DE] text-white border border-blue-300">Nomor Telepon</fwb-table-head-cell>
          <fwb-table-head-cell class="px-4 py-3 bg-[#4954DE] text-white border border-blue-300">Nama Lomba</fwb-table-head-cell>
          <fwb-table-head-cell class="px-4 py-3 bg-[#4954DE] text-white border border-blue-300">Tanggal Daftar</fwb-table-head-cell>
          <fwb-table-head-cell class="px-4 py-3 bg-[#4954DE] text-white border border-blue-300">Status Pembayaran</fwb-table-head-cell>
           <fwb-table-head-cell class="px-4 py-3 bg-[#4954DE] text-white border border-blue-300">Jumlah Pembayaran</fwb-table-head-cell>
        ...
        <fwb-table-body>
          ...
          <!-- Loop data peserta -->
          <fwb-table-row 
            v-for="peserta in pesertaList" 
            :key="peserta.id_riwayat"
  ...
  {{ peserta.nomor_telepon || '-' }}
    ...
    <div v-if="!isLoading" class="mt-6 p-2 bg-gray-50 rounded-lg">
      <div class="flex items-center">
        ...
        <p class="text-sm text-gray-600">Total peserta yang telah mendaftar: <span class="font-semibold">{{ totalPeserta }} orang</span></p>
      </div>
    </div>
...
:deep(th),
:deep(td) {
  border: 1px solid #93c5fd !important; /* garis antar kolom */
}
